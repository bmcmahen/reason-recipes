...
let string_of_cursor = (cursorStyle) =>
  switch cursorStyle {
  | Pointer => "pointer"
  };
...
let string_of_style = (style) =>
  switch style {
  | Cursor(cursorStyle) => "cursor:" ++ string_of_cursor(cursorStyle)
  ...
  | BoxShadow(string) => "box-shadow:" ++ string
  /* transform styles */
  ...
  | BackgroundImage(string) => "background-image:" ++ string
  ...
  | FontSize(dimension) => "font-size:" ++ string_of_dimension(dimension)
